...
- **Feasibility:** high for L0–L3 (mechanical), medium for L4 (needs paraphrase generation — an LLM labeler can pre-generate the euphemism set offline).
...
- **Controls:** this plan *is* the control layer for t13 — matcher ensemble (review note C), probe ground truth (review note E), `contested_rate` reporting. Until probes run, every laundering number is explicitly "matcher-dependent, end-to-end".
...
- **Controls:** linear vs MLP probe directly tests the t18 linear-separability assumption (review note P1#3). Class-balanced via `auto_select_concepts`; baseline reported. The probe IS the ground truth that disentangles NLA encoding from AV verbalization — the central DESIGN_REVIEW P0 fix applied to ARiES.
